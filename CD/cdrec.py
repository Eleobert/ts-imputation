...
def main():
    matrix = np.loadtxt("../CD/data_miss.txt")
    recovered = centroid_recovery(matrix, 1)
    reference = np.loadtxt("../CD/data_full.txt");
    print(np.corrcoef(reference, rowvar=False))
    
    print("Recovery error:")
    print(np.linalg.norm(recovered - reference))
    #np.savetxt("data_recov.txt", recovered, fmt="%10.5f")

    # matrix = [[-4.0, 1, 3], [-1, 3, -1], [2, 6, 6], [5, 5, 3]]
